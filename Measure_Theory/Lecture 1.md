...
Define $\int_{a}^{\overline{b}} f(x)dx = \inf_{P} U(P, f)$
...
→ let $\{r_1, r_2, \dots \}$ be a set of rationals in $[0, 1]$
...
$[c, d] \subseteq [a, b]$, $\int_a^b \chi_{[c,d]}(x)dx = \int_c^d dx = d - c$ ]
