...
i) (monotonically). If $E, F \in \mathcal{F}$ with $E \subset F$ then $\mu(E) \leq \mu(F)$

ii) (subadditivity). $E_1, E_2, \dots \in \mathcal{F}$ then $\mu(\bigcup_{j=1}^{\infty} E_j) \leq \sum_{j=1}^{\infty} \mu(E_j)$

iii) (continuity from below). If $\{ E_j \} \subseteq \mathcal{F}$ with $E_j \uparrow E$ that is $E_j \subseteq E_{j+1}$ and $E = \bigcup_{j=1}^{\infty} E_j$

Then $\lim_{j \to \infty} \mu(E_j) = \mu(E)$.

iv) (Continuity from above):- If $\{ E_j \} \subseteq \mathcal{F}$ with $E_j \downarrow E$. That is, $E_{j+1} \subseteq E_j$ and $E = \bigcap_{j=1}^{\infty} E_j$ and $\mu(E_1) < \infty$. Then $\mu(E) = \lim_{j \to \infty} \mu(E_j)$.
...
iii) Set $F_k = E_k - E_{k-1}, k \geq 2, F_1 = E_1$. Then $\{ F_k \}$ is a disjoint collection in $\mathcal{F}$ with $\bigcup_{k=1}^{n} F_k = E_n \implies E = \bigcup_{k=1}^{\infty} F_k$.

$\therefore \mu(E) = \sum_{k=1}^{\infty} \mu(F_k) = \lim_{n \to \infty} \sum_{k=1}^{n} \mu(F_k) = \lim_{n \to \infty} \mu(E_n)$.

iv) Set $G_k = E_1 - E_{k+1}, k \geq 1$; Then $G_k$ is an increasing sequence to $E_1 - \bigcap_{k \geq 1} E_k$.
...
i) $\mu(A) = \gamma(A), \forall A \in \mathcal{S}$

ii) If there exist two additive (respectively $\sigma$-additive) set functions $\gamma_1$ and $\gamma_2$ on $\mathcal{A}(\mathcal{S})$ satisfying (i) then $\gamma_1(E) = \gamma_2(E) \ \forall E \in \mathcal{A}(\mathcal{S})$.

### Proof

If $A \in \mathcal{A}(\mathcal{S})$ then $A = \bigcup_{j=1}^{n} A_j$ for $A_j \in \mathcal{S}, j = 1, \dots, n$
...
Therefore, $\gamma(A) = \sum_{j=1}^{n} \mu(E_j) = \sum_{j=1}^{n} \sum_{k=1}^{m} \mu(E_j \cap F_k)$

$$= \sum_{k=1}^{m} \sum_{j=1}^{n} \mu(E_j \cap F_k) = \sum_{k=1}^{m} \mu(F_k)$$
